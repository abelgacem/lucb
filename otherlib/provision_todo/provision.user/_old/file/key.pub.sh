...
siFileNameDst="authorized_keys"
siFileFolderDst="/home/${siUserOsUsed}/.ssh"
siFilePathDst="${siFileFolderDst}/${siFileNameDst}"
## Section.Debug
...
[ -z ${siOtpId} ] && { printf "    - Error %-48s > Missing > Id or Name\n" "${sDebugPath}" ; exit; }
...
sCheck01="$(${sAction})"
sAction="ssh ${siVmName} sudo wc -l ${siFilePathDst} |  cut -d' ' -f1"
sCheck02="$(${sAction})"
printf "    - Check %-48s > Exists > [TODO > Not > Idempotent] > %s\n" "${sDebugPatho}" "(${sCheck02})${sCheck01}"