...
sAction="ssh ${siVmName} lsb_release -rs"
sVersion="$(${sAction})"
# Step
cat <<EOF | ssh ${siVmName} sudo tee ${siFilePath} >/dev/null
# ${siFilePath}
deb http://download.opensuse.org/repositories/devel:/kubic:/libcontainers:/stable/xUbuntu_${sVersion}/ /
EOF

# Step > Add > Key.apt
sAction="ssh ${siVmName}  sudo wget --quiet -O - https://download.opensuse.org/repositories/devel:kubic:libcontainers:stable/xUbuntu_${sVersion}/Release.key | sudo apt-key add -"
...
# curl -L "https://download.opensuse.org/repositories/devel:/kubic:/libcontainers:/stable/xUbuntu_${lOsVersion}/Release.key" | sudo apt-key add - ;
