...
sAction="ssh ${siVmName} sudo sed -i 's/^SELINUX=.*$/SELINUX=permissive/' ${siFilePath}"
#echo -e "      - Check ${sDebugPath} > File Exists > ${sCheck}"
${sAction}

# Step > Provision.Session
sAction="ssh ${siVmName}  sudo setenforce 0"
#echo -e ${sAction}
${sAction}
...
# Check > Action
sAction="ssh ${siVmName} sudo grep -i SELINUX=perm ${siFilePath}"
sCheck="$(${sAction})"
#echo -e "      - Check ${sDebugPath} > File Exists > ${sCheck}"
