# shortcut
luc_install_buildah() { luc_buildah_install "$@"; }

# purpose additional install specific to os:alpine
# args: NONE
luc_buildah_alpine_install() {
...
luc_buildah_install() {
  # define var
  local lCLI="buildah"
  local lTOOL_WKSPC="$luc_EV_JEKYLL_FOLDER_WKSPC"
  ...
  case "$lDISTRO" in
    "alpine")          luc_buildah_alpine_install ;;
    "ubuntu"|"debian") luc_buildah_ubuntu_install ;;
  ...
  # create root tool home 
  luc_core_echo "step" "create jekyll WORKSPACE: $lTOOL_WKSPC"
  mkdir -p "$lTOOL_WKSPC"

  # install jekyll gem dependencies 
  luc_core_echo "step" "create dummy site to install jekyll site std dependency"
  lECHOVAL=$(luc_buildah_site_create --temp > /dev/null); lRETVAL=$?
