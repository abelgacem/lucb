# purpose: buildah create a jekyll image from a ruby image
# args: <RUBY_IMAGE_SID>
luc_buildah_image_jekyll_create_from()  {
  local lMSG_PURPOSE="buildah create a jekyll image from a ruby image"  
  ...
  local lLABEL_KEY_IMAGE_02="${luc_EV_BUILDAH_LABEL_KEY_IMAGE_02}"
  local lLABEL_KEY_IMAGE_CURRENT="${luc_EV_BUILDAH_LABEL_KEY_IMAGE_CURRENT}"
  local lLABEL_KEY_SERVER_PORT="${luc_EV_BUILDAH_LABEL_KEY_SERVER_PORT}"
  ...
  lIMAGE_DST_FULLNAME="dev/jekyll:${lIMAGE_SRC_TAG}"

  ###### DEFINE CONFIGURATION ###########################
  lLABELS=" 
    $lLABEL_KEY_IMAGE_02=$lIMAGE_SRC_FULLNAME
    $lLABEL_KEY_IMAGE_CURRENT=$lIMAGE_DST_FULLNAME
    $lLABEL_KEY_SERVER_PORT=$lLABEL_VALUE_SERVER_PORT
  "
  lPORT=" --port $lLABEL_VALUE_SERVER_PORT"
  ...
  lCLI=". \$LUCSETUP ; luc_jekyll_install" # uses envar $LUCSETUP already define in the container
...
###### CHECK PROVISIONING ############################
  luc_core_echo "chec" "JEKYLL is provisioned inside the container"
  luc_buildah_container_cli_run $lCONTAINER_TEMP_SID 'echo "jekyll version: $(jekyll -v) - bundle version: $(bundle -v)"'
  ###### CONFIGURE CONTAINER ###########################
  luc_core_echo "step" "set container ENVARs, LABELs, USER and WORKDIR"
  buildah config ${lLIST_ENVAR} ${lLIST_LABEL} $lPORT $lUSER $lWKDIR ${lCONTAINER_TEMP_SID}

  ###### CHECK CONGIGURATION ###########################

  luc_core_echo "chec" "PORT value in container metadata"
  luc_buildah_property_get --container $lCONTAINER_TEMP_SID PORT | jq .
