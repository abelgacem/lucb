# purpose: buildah create a ruby image from a base os image
# args: <BASE_IMAGE_SID>
luc_buildah_image_ruby_create_from()  {
  local lMSG_PURPOSE="buildah create a ruby image from a base os image"  
  ...
  local lLABEL_KEY_IMAGE_01="${luc_EV_BUILDAH_LABEL_KEY_IMAGE_01}"
  ...
  # getorexit image PATH
  lECHOVAL=$(luc_buildah_property_get --image  ${lIMAGE_SRC_SID} ENVAR:PATH); lRETVAL=$?  
  [ 0 -ne "$lRETVAL" ]  && echo "$lECHOVAL" && return 1 || lIMAGE_ENVAR_PATH="$lECHOVAL"

  # define image fullname
  lIMAGE_DST_FULLNAME="dev/ruby:${lIMAGE_SRC_SNAME/linux/}"

  ###### DEFINE CONFIGURATION ###########################
  lENVARS="GEM_HOME=/home/${lOS_USER_SUDO}/wkspc/rubygems PATH=/home/${lOS_USER_SUDO}/wkspc/rubygems/bin:$lIMAGE_ENVAR_PATH"
  lLABELS=" 
  ...
  lCLI=". \$LUCSETUP ; luc_ruby_install" # uses envar $LUCSETUP already define in the container
  ...
  ###### CHECK PROVISIONING ############################
  luc_core_echo "chec" "RUBY is provisioned inside the container"
  luc_buildah_container_cli_run $lCONTAINER_TEMP_SID 'echo "ruby version: $(ruby -v) - gem version: $(gem -v) - bundle version: $(bundle -v)"'
  ###### CONFIGURE CONTAINER ###########################
